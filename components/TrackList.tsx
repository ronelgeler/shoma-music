...
export default function TrackList({ tracks, onDelete }: TrackListProps) {
  ...
  return (
    <div className="w-full text-left text-neutral-400 text-sm">
      <div className="grid grid-cols-[40px_1fr_1fr_40px] gap-4 p-3 border-b border-neutral-800 font-medium">
        <div>#</div>
        <div>Title</div>
        <div>Album</div>
      ...
      <div className="flex flex-col mt-2 space-y-1">
        {tracks.map((track, index) => {
          ...
          return (
            <div 
              key={track.uid} 
              className={`grid grid-cols-[40px_1fr_1fr_40px] gap-4 p-3 rounded-md hover:bg-neutral-800/50 group transition ${isCurrent ? 'bg-neutral-800/30 text-green-500' : ''}`}
            >
